...
def get_data(
    ...
    interval: str = Query(
        ..., description="The time interval for the data (e.g., '1m', '5d', '1wk')."
    ),
):
    ...
    if data.empty:
        return JSONResponse(
            content={"error": "No data found for the given parameters."},
            status_code=404,
        )
    ...
    for _, row in data.iterrows():
        ...
        event = {
            "time_object": {
                "timestamp": {
                    "": row["Date"]
                },  # Ensure the timestamp follows the expected format
                "duration": 1,
