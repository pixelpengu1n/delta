...
from main import \
    app  # Replace with your actual FastAPI app file if it's not named main.py
...
def test_collect_valid_csv():
    ...
    assert (
        first_event["attribute"]["value"] == 100
    )  # Should remain int or str, depending on CSV parser
...
def test_collect_csv_without_event_type_column():
    ...
    response = client.post(
        "/collect/", files=file, params={"timestamp_column": "timestamp"}
    )
